...
export default function Archives() {
  ...
  const [openIssueId, setOpenIssueId] = useState(issues[0]?.id ?? '');
  const content = archivesContent[currentLang];

  useEffect(() => {
    const hashIssueId = window.location.hash.replace('#', '');

    if (hashIssueId && issues.some((issue) => issue.id === hashIssueId)) {
      setOpenIssueId(hashIssueId);
    }
  }, []);
            ...
            <div className="max-w-5xl mx-auto space-y-8">
              {issues.map((issue) => (
                <section
                  key={issue.id}
                  id={issue.id}
                  className="bg-white rounded border border-border scroll-mt-24 overflow-hidden"
                >
                  <button
                    type="button"
                    className="w-full text-left p-6 md:p-8"
                    onClick={() => setOpenIssueId(openIssueId === issue.id ? '' : issue.id)}
                    aria-expanded={openIssueId === issue.id}
                    aria-controls={`${issue.id}-articles`}
                  >
                    ...
                    <div className="flex items-center gap-6 text-sm text-muted-foreground">
                      ...
                      <ChevronDown
                        ...
                        className={`text-accent transition-transform ${
                          openIssueId === issue.id ? 'rotate-180' : ''
                        }`}
                      />
                    </div>
                    </div>
                  ...
                  <div
                    id={`${issue.id}-articles`}
                    className={`border-t border-border divide-y divide-border px-6 md:px-8 ${
                      openIssueId === issue.id ? 'block' : 'hidden'
                    }`}
                  >
